...
const evalClass = (exp: ClassExp, env: Env): Result<Value> => {
    const evalFields = mapResult((field: VarDecl) => applyEnv(env,field.var), exp.fields);
    const evalMethods = mapResult((method: Binding) =>
    bind(L3applicativeEval(method.val, env), (methodVal: Value) =>
        makeOk({ var: method.var.var, val: methodVal })
    ), exp.methods);
    //not done yet, you need to return something!!!
    }
