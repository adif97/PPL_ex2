...
import {isPrimOp, CExp, PrimOp, VarDecl, ClassExp, isClassExp} from './L3-ast';
...
export const isSExp = (x: any): x is SExpValue =>
    ...
    isSymbolSExp(x) || isCompoundSExp(x) || isEmptySExp(x) || isPrimOp(x) || isClosure(x); //|| isClassExp(x);
...
//export const classToString = (c: ClassExp): string =>
...
export const valueToString = (val: Value): string =>
    ...
    //isClassExp(val) ? classToString(val) :
    val;
